...
- **内置单价抄录自 2026-09-16 的官方页面**：[Anthropic 定价](https://platform.claude.com/docs/en/about-claude/pricing)与 [OpenAI 定价](https://developers.openai.com/api/docs/pricing)；任一方调价后即会过期。
- 每个数字都直接抄自官方表格，而非用倍率推算——因为倍率存在例外：Claude Fable 5.1 与 Mythos 5.1 的缓存读取是基础输入价的 0.025 倍，而不是通常的 0.1 倍。
- Claude 的缓存 token 按各自费率计价：读取为基础输入价的 0.1 倍，写入为 1.25 倍（5 分钟 TTL）或 2 倍（1 小时 TTL）。Claude 日志会分别记录这两种写入 TTL，因此按实际拆分计价；若某条记录没有提供拆分，则按较便宜的 5 分钟费率计算。
- OpenAI 未公布缓存写入的溢价——只有缓存输入享受折扣，其余按基础输入价计费——因此这些模型的两种写入费率都等于输入价。
- **少数模型 id 没有公开单价**，保持「未定价」：`gpt-5-codex`、`gpt-5.2-codex` 和 `codex-auto-review` 在两个定价页面上都查不到。匹配方式为精确 id，或去掉结尾 `-YYYYMMDD` 快照日期后的 id；**刻意不做**前缀回退：`gpt-5-codex` 不是 `gpt-5`，且 codex 变体的定价与其名字相近的基础模型并不相同。
