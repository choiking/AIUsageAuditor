...
swift build -c release --product AIUsageAuditor --scratch-path "$BUILD_ROOT/spm" --cache-path "$BUILD_ROOT/cache" "$@"
...
APP="$PROJECT_DIR/build/AI Usage Auditor.app"
mkdir -p "$APP/Contents/MacOS"
cp "$BIN_DIR/AIUsageAuditor" "$APP/Contents/MacOS/AIUsageAuditor"
cp Config/Info.plist "$APP/Contents/Info.plist"
/usr/libexec/PlistBuddy -c 'Set :CFBundleExecutable AIUsageAuditor' "$APP/Contents/Info.plist"
/usr/libexec/PlistBuddy -c 'Set :CFBundleIdentifier local.aiusageauditor.app' "$APP/Contents/Info.plist"
